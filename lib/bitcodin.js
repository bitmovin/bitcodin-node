...
Bitcodin.prototype = {
    ...
    sprite : {
        get: function(spriteId) {
            return rest.get('/sprite/' + spriteId);
        },
        create: function(sprite) {
            var def = Q.defer();

            rest.post('/sprite', sprite).then(
                function(response) {
                    (function poll() {
                        rest.get('/sprite/' + response.id).then(
                            function (response) {
                                switch(response.state) {
                                    case 'FINISHED':
                                        def.resolve(response);
                                        break;
                                    case 'ERROR':
                                        def.reject("Error while creating sprite!");
                                        break;
                                    default:
                                        setTimeout(poll, 5000);
                                }
